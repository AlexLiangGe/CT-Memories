...
const MsgScrollComp = () => {
  ...
  function getRandomDateWithinLastSixMonths() {
    ...
    // 计算当前日期与半年前日期之间的毫秒差
    const diffTime = now.getTime() - sixMonthsAgo.getTime();

    // 生成介于0和这个时间差之间的随机毫秒数
    const randomDiff = Math.random() * diffTime;

    // 用这个随机毫秒数加上半年前的日期得到随机日期
    const randomDate = new Date(sixMonthsAgo.getTime() + randomDiff);
  ...
  function generateRandomAvatarUrl() {
    ...
    return `http://liangge.nihe.cc/image/${index}.webp`; // 请替换https://example.com为实际头像存储地址
  }
      ...
      {messages.map((msg, index) => (
        <div key={index}>
          <div className="message-header">
            ...
            />
            <div>
              <div className="name">{generateRandomName()}</div>
              <div className="time">{formatDate(getRandomDateWithinLastSixMonths(index))}</div>
            </div>
          </div>
